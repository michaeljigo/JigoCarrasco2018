% anal_thresh(subj,expName,threshRun,nSets,varargin)
...
for s = 1:nSets
    ...
    targTilt.val = threshTargTilt;
    targTilt.label = {'vertical' 'tilted'};
    ...
    try
        ...
        % interval analysis
        tiltEcc = condParser(acc,targTilt,targEcc);
        figure(intFig);
        subplot(nRows,nCols,s);
        lineCol = 'kb';
        for i = 1:size(tiltEcc.perf,1)
            plotLine(i) = plot(plotEcc,tiltEcc.perf(i,:),[lineCol(i),'.-']); hold on
        end
        legend(plotLine,targTilt.label);
        set(gca,'YLim',[0.45 1]);
        set(gca,'XLim',[-0.1 8]);
      catch
    ...
    corrIdx = find(setData.stimVal==thresh(s) & setData.response==setData.targetTilt); 
    ...
    if threshRun==2
        ...
        orgPCorr = setData.response==setData.targetTilt;
        orgPCorr = mean(orgPCorr(setData.stimVal==finalThresh & setData.targetEcc<removeEcc));
