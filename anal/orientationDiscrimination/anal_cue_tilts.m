...
function anal_cue_tilts(subj,expName,nSets)
...
tiltFig = figure('Name',[subj, ' tilts']);
stair = figure('Name',subj);
...
for s = 1:nSets
    ...
    % trial accuracy
    acc = setData.response==setData.targetTilt;
    ...
    % tilt condition
    tilt.val = setData.tiltDir;
    tilt.label = {'CCW' 'CW'};
    
    % vert or tilted
    VoT.val = setData.targetTilt;
    VoT.label = {'vertical' 'tilted'};
    ...
    % tilt RT analysis
    rtTilt = condParser(setData.reactionTime,VoT,tilt,cue,ecc,options);
    
    % proportion correct for different cueing conditions split for CW and
    % CCW tilt
    pCorrTilt = condParser(acc,VoT,tilt,cue,ecc,options);
    ...
    for c = 1:size(pCorrEcc.raw,1)
        ...
        % tilts
        for i = 1:size(pCorrTilt.raw,1)
            tempCI = sort(cell2mat(squeeze(pCorrTilt.bootDistribution(i,c,:))'));
            ci.tilt{c,i} = tempCI(ciBounds,:);
            tempCI = sort(cell2mat(squeeze(rtTilt.bootDistribution(i,c,:))'));
            ci.rtTilt{c,i} = tempCI(ciBounds,:);
        end
        
    end
    
    % get staircase results
    stairRes = condParser(setData.stimVal,VoT,tilt);
    ...
    sig = pval_pCorr>0.95|pval_pCorr<0.05;
...
%     % plot tilt rt
%     for i = 1:size(rtTilt.perf,1)
%         % plot the results of each interval in separate subplots
%         subplot(nSets,2,2*(s-1)+i);
%         tiltData = squeeze(rtTilt.perf(i,:,:));
%         for c = 1:size(tiltData,1)
%             tiltLine(c) = plot(x,tiltData(c,:),['.',lineCol(c),'-']); hold on
%             errorbar(x,tiltData(c,:),abs(ci.rtTilt{c,i}(1,:)-tiltData(c,:)),...
%                 abs(ci.rtTilt{c,i}(2,:)-tiltData(c,:)),'Linestyle','none',...
%                 'color',lineCol(c));
%         end
%         set(gca,'XLim',[-0.1 8]);
%         set(gca,'YLim',[0 1.05]);
%         title(rtTilt.factorLabels.factor1{i});
%         legend(hemiLine,rtTilt.factorLabels.factor2);
%         xlabel('eccentricity'); ylabel('proportion correct');
%     end
    
        % plot TILT results
    figure(tiltFig); plotn = 0;
    pCorrTilt.factorLabels.factor1 = {'vertical' 'CCW' 'CW'};
    for i = 1:size(pCorrTilt.perf,1)
        % plot the results of each tilt (vertical/CCW/CW) in separate
        % subplots
        
        if strcmp(pCorrTilt.factorLabels.factor1{i},'vertical')
            tiltData = squeeze(mean(squeeze(pCorrTilt.perf(i,:,:,:)),1));
        else
            tiltData = squeeze(pCorrTilt.perf(i,:,:,:));
        end
        orgTiltData = tiltData;
        
        for t = 1:length(size(orgTiltData))-1
            plotn = plotn+1;
            subplot(nSets,3,3*(s-1)+plotn);
            if length(size(orgTiltData))==3
                tiltData = squeeze(orgTiltData(t,:,:));
            end
            for c = 1:size(tiltData,1)
                tiltLine(c) = plot(x,tiltData(c,:),['.',lineCol(c),'-']); hold on
%                 errorbar(x,tiltData(c,:),abs(ci.tilt{c,i}(1,:)-tiltData(c,:)),...
%                     abs(ci.tilt{c,i}(2,:)-tiltData(c,:)),'Linestyle','none',...
%                     'color',lineCol(c));
            end
            set(gca,'XLim',[-0.1 8]);
            set(gca,'YLim',[0 1.05]);
            title(pCorrTilt.factorLabels.factor1{i});
            legend(hemiLine,pCorrTilt.factorLabels.factor2);
            xlabel('eccentricity'); ylabel('proportion correct');
        end
    end
    ...
    figure(stair);
    subplot(nRows,nCols,s);
    for i = 1:length(stairRes.raw)
        thisStair = stairRes.raw{i};
        stairLine(i) = plot(thisStair,[lineCol(i),'-']); hold on
    end
    title('staircase');
    legend(stairLine,tilt.label);
    xlabel('trials'); ylabel('tilt (deg');
