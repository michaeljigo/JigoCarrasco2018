...
function stats = analBothCues(exclude,doFit,figExt)
...
for t = 1:length(expName)
    ...
    for s = 1:length(subj)
        ...
        rawPCorr(s,t,:,:) = pCorrEcc.perf;
        rawRT(s,t,:,:) = rtEcc.perf;
        rawStimVal(s,t,:,:) = stimVal.perf;
        rawCueEffect(s,t,:) = diff(pCorrEcc.perf,1);
    end
end

%% Do group analyses
% prop. corr: Task x Cue x Eccentricity ANOVA
temp = buildrmaov3Matrix(rawPCorr);
stats.pCorr.taskXcueXecc = rmaov33(temp,'printOut=0');

% rt: Task x Cue x Eccentricity ANOVA
temp = buildrmaov3Matrix(rawRT);
stats.rt.taskXcueXecc = rmaov33(temp,'printOut=0');

% contrast: Cue x Eccentricity ANOVA
temp = buildrmaov3Matrix(rawStimVal);
stats.contrast.taskXcueXecc = rmaov33(temp,'printOut=0');
...
cueEff = squeeze(diff(rawPCorr,[],3));
    ...
    if doFit.y
        ...
        switch doFit.fittype
            case 'polynomial'
                ...
                % compute r2 of fit
                r2 = calcR2(y,polyval(fitCoeff,ecc),polyOrder);
            case 'spline'
                nTries = 2;
                for n = 1:nTries
                    if n==1
...
if doFit.y
title(sprintf('cue=%.3f; ecc=%.3f; cueXecc=%.3f; exoR2=%.2f; endoR2=%.2f',...
    stats.cueEffect.iv1{3},stats.cueEffect.iv2{3},stats.cueEffect.iv12{3},...
...
saveas(subjScatter,[figFolder,'exoVsEndoSubj',figExt,'.tif']);
if doFit.y
    saveas(cueEffect,[figFolder,'exoVSendoCueEffect_',doFit.fittype,figExt,'.tif']);
else
saveas(cueEffect,[figFolder,'exoVSendoCueEffect',figExt,'.tif']);
end