...
function myscreen = main_texDetect_VoT(subj,thresh,eccSet,nSets,expName,eyeTrack)
...
if thresh
    ...
    switch thresh
        case 1 % staircase session
            ...
            stimulus.trialnum = 84; %84, should take ~3 minutes/block
...
for n = 1:nBlocks
    ...
    if thresh
        ...
        elseif isodd(n)
            ...
            if thresh==1
                stairParams.lastPosterior = normpdf(stairParams.alphaRange,0.85,0.2);
            end
...
%         end
        ...
        % run task
        runContrastThresh_VoT(myscreen);
    else
        % run the experiment
        askIfReady(thresh,exoOrEndo)
        % display target locations
