...
function runExoTexDetect_incDec(myscreen)
...
stimulus.exoCue.yDistFromTarg = stimulus.tex.texParams.gaborSD+0.3; % this ensures that the bar will be 0.3 degrees above one standard deviation of gabor
...
if ~isfield(stimulus,'targetIncDec')
    stimulus.targetIncDec = [1 2]; %[1=icnrement; 2=decrement]
...
task{1}.parameter.targetIncDec = stimulus.targetIncDec; % [increment decrement]
task{1}.random = 1;
...
c = find(stimulus.tex.texParams.contrast==stimulus.gabor.contrast(task.thistrial.targetIncDec));
...
if task.thistrial.thisseg==3 && stimulus.eye.eyeTrack
    % takes average position during the fixation period (segment 2) and
    % uses that as the new zero position
