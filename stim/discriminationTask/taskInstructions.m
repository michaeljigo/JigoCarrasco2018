...
mglTextDraw('1 (on number pad): Target in 1st interval',[0 0.5]);
mglTextDraw('2 (on number pad): Target in 2nd interval',[0 -0.5]);
...
stimulus.exoCue.rMin = 0.2; % starting point of line
stimulus.exoCue.rMax = 0.4; % extent of line
...
mglTextDraw('We will just use the central cue to explain the task.',[0 5]);
...
% Target presentation
mglTextDraw('After the first cue is presented, a texture stimulus will appear.',[0 7]);
mglTextDraw('If present, the target will appear in the cued location.',[0 6]);
cd('./textures');
target = make2ndOrderTex(2,0.25,1,2.4,2,1,'dispFig=0','newCarrier=1');
noise = make2ndOrderTex(2,0.25,1,2.4,2,0,'dispFig=0','newCarrier=1');
cd(orgDir);
target = mglCreateTexture(target);
noise = mglCreateTexture(noise);
mglBltTexture(target,[0 0]);
mglFixationCross(stimulus.fixation.width,stimulus.fixation.size,...
    stimulus.fixation.color,stimulus.fixation.loc);
mglFlush; pause; mglClearScreen;

mglTextDraw('Then another cue will appear.',[0 7]);
% display number at origin
mglTextDraw(num2str(0),...
    [stimulus.respCue.xNum,stimulus.respCue.yNum]);
mglFlush; pause; mglClearScreen;

% second interval
mglTextDraw('Followed by another texture stimulus.',[0 7]);
mglTextDraw('If present, the target will appear in the cued location. The target will only be present in one interval.',[0 6]);
mglBltTexture(noise,[0 0]);
mglFixationCross(stimulus.fixation.width,stimulus.fixation.size,...
    stimulus.fixation.color,stimulus.fixation.loc);
mglFlush; pause; mglClearScreen;
...
% Response cue
mglTextDraw('Following the intervals, the fixation cross will turn green. This is when you can respond',[0 7]);
mglTextDraw('Press 1 if the target appeared in the first interval.',[0 6]);
mglTextDraw('Press 2 if it appeared in the second interval.',[0 5]);
mglTextDraw('Feel free to blink when you respond.',[0 4]);
mglFixationCross(stimulus.fixation.width,stimulus.fixation.size,...
    [0 0.25 0],stimulus.fixation.loc);
mglFlush; pause; mglClearScreen;
