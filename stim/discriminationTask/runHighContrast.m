...
task{1}.numTrials = 28; % 16 trials/ecc/cue
...
task{1}.parameter.targetTilt = [1 2]; % [CCW CW]
task{1}.random = 1;
...
% determine accuracy for each finger
% if task.thistrial.targetHemi==1
%     thisTilt = task.thistrial.targetTilt;
% elseif task.thistrial.targetHemi==2
%     thisTilt = task.thistrial.targetTilt+2;
% end
% if task.thistrial.targetEcc==0
%     resp = find(task.thistrial.buttonState);
%     if any([1 3]==resp)
%         % CCW
%         thisTilt = 1;
%     elseif any([2 4]==resp);
...
%         thisTilt = 2;
%     end
%     if thisTilt==task.thistrial.targetTilt
%         correct = 1;
%     else
%         correct = 0;
%     end
% else
%     if find(task.thistrial.buttonState)~=thisTilt
%         correct = 0;
%     else
%         correct = 1;
%     end
% end
% stimulus.acc(task.trialnum) = correct;
