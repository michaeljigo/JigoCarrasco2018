...
function runEndoTexDetect(myscreen)
...
task{1}.parameter.targetTilt = stimulus.targetTilt; % [CCW CW]
task{1}.random = 1;
...
[task{1}, myscreen] = initTask(task{1},myscreen,@startSegmentCallback,...
    @screenUpdateCallback,@f,@startTrialCallback,...
    @endTrialCallback);
...
switch task.thistrial.targetTilt
    case 1
        % CCW
        gaborAngle = stimulus.gabor.tiltOffset + 90;
    case 2
        % CW
...
temp = make2ndOrderTex(stimulus.carrier.sf,stimulus.gabor.sf,...
    ...
    task.thistrial.targetHemi,1,'gaborAngle',gaborAngle,'gaborSD',stimulus.gabor.sd);
