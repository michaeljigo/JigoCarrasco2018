...
function runContrastThresh_incDec(myscreen)
...
task{1}.seglen = [eyeContingent fixation cue cueISI stim respISI resp];
...
task{1}.parameter.targetIncDec = [1 2]; % [increment decrement]
task{1}.random = 1;
task{1}.randVars.uniform.carrierFlip = 1:size(stimulus.tex.textures,1)/2;
...
if isfield(stimulus.tex,'target')
    mglDeleteTexture(stimulus.tex.target);
end

% index target texture to be displayed during the trial
switch task.thistrial.targetIncDec
    case 1 %increment
        f = size(stimulus.tex.textures,1)/2+task.thistrial.carrierFlip;
        c = find(stimulus.tex.texParams.contrast==stimulus.stair.inc.xCurrent);
    case 2 % decrement
        f = task.thistrial.carrierFlip;
        c = find(stimulus.tex.texParams.contrast==stimulus.stair.dec.xCurrent);
end
...
stimulus.tex.target = mglCreateTexture(stimulus.tex.textures{f,h,e,c});
...
switch task.thistrial.thisseg
    ...
    case {4 6}
        ...
        mglFixationCross(stimulus.fixation.width,stimulus.fixation.size,...
            stimulus.fixation.color,stimulus.fixation.loc);
    case 5 % texture
        mglBltTexture(stimulus.tex.target,[0 0]);
    case 7 % response cue
...
if find(task.thistrial.buttonState)~=task.thistrial.targetIncDec
...
if task.thistrial.targetIncDec==1
    stimulus.stimVal(task.trialnum) = stimulus.stair.inc.xCurrent;
elseif task.thistrial.targetIncDec==2
    stimulus.stimVal(task.trialnum) = stimulus.stair.dec.xCurrent;
end

if task.thistrial.targetEcc<1
    switch task.thistrial.targetIncDec
        case 1
            % update increment staircase
            stimulus.stair.inc = usePalamedesStaircase(stimulus.stair.inc,correct);
        case 2
            % update decrement staircase
            stimulus.stair.dec = usePalamedesStaircase(stimulus.stair.dec,correct);
    end
end
