...
function runExoTexDetect_detection(myscreen)
...
stimulus.exoCue.yDistFromTarg = stimulus.gabor.sd+(stimulus.gabor.sd); % 1
...
task{1}.numTrials = 112; % 16 trials/ecc/cue
...
task{1}.parameter.targetPresent = [1 2]; % [present absent]
task{1}.random = 1;
...
temp = make2ndOrderTex(stimulus.carrier.sf,stimulus.gabor.sf,...
    ...
    'gaborSD',stimulus.gabor.sd,'gaborPhase',180);
