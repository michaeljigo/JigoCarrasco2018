...
if all(cellfun('isempty',expOrder(subjIdx).toDo.cue))
    ...
    % get the current eccentricity set
    thisEcc = expOrder(subjIdx).toDo.ecc{currentIdx}(1);
end

if ~initializeSubj
    fprintf('====== %s cue (eccSet #%i) ======\n',thisCue,thisEcc);
    main_texDetect(subj,0,thisEcc,12,[expPrefix,'_',thisCue],eyeTrack,workspaceTex);
    ...
    save(filename,'subjList','subjList','expOrder');
end