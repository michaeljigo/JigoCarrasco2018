% This function adjusts the stimulus presentation duration in steps of 100
% ms such that the subject can see the target and get used to looking at
% detecting the target in smaller and smaller durations.

function runStimDurChange(myscreen,workspaceTex)
global stimulus

%% Setup
% set up the staircase
stairParams.whichStair = 1; % using best PEST
stairParams.alphaRange = 0.1:0.025:0.5;
stairParams.fitBeta = 2;
stairParams.fitLambda = 0.1;
stairParams.fitGamma = 0.1;
stairParams.threshPerformance = 0.4;
stairParams.PF = 'arbWeibull';
stairParams.lastPosterior = normpdf(stairParams.alphaRange,0.5,0.25);
...
expParams.eccentricity = stimulus.gabor.ecc;
...
if isempty(workspaceTex)
    ...
    for t = 1:length(tex)
        ...
        %     evalc(['texture',num2str(t),'= texture;']);
    end
    disppercent(inf);
...
isi = stimulus.soa;
stim = inf; % adjusted by staircase
respIsi = 0.2;
respCue = inf;
minIti = 0.1;
...
task{1}.numTrials = inf; %arbritrarily set for now

% parameters
task{1}.parameter.targetEcc = 2.4; %fixed @ 2.4
...
if task.thistrial.targetPresent
    ...
    c = find(stimulus.tex.texParams.contrast==1); % fixed at highest contrast
else
...
if task.thistrial.thisseg==5
    stimulus.stimDurTimer = mglGetSecs;
end
...
switch task.thistrial.thisseg
    ...
    case 3 % cue
        ...
        mglFixationCross(stimulus.fixation.width,stimulus.fixation.size,...
            stimulus.fixation.color,stimulus.fixation.loc);
    case 5 % texture
        if mglGetSecs(stimulus.stimDurTimer)<=stimulus.stair.xCurrent
            mglBltTexture(stimulus.tex.thisTexture,[0 0]);
        else
            task = jumpSegment(task);
        end
        % draw fixation cross to stop subjects' eyes from wandering
        mglFixationCross(stimulus.fixation.width,stimulus.fixation.size,...
            stimulus.fixation.color,stimulus.fixation.loc);
    case 7 % response cue
        % display number at origin
...
fprintf('trial #%i: %.1f\n',task.trialnum,stimulus.stair.xCurrent);
updateStair = 0;
...
if task.thistrial.targetPresent
    ...
    % when the staircase first reaches 100ms, stop it from updating
    if length(stimulus.stair.x)>=20 && ...
            all(stimulus.stair.x(end-19:end)==min(stimulus.stair.priorAlphaRange))
       updateStair = 0;
