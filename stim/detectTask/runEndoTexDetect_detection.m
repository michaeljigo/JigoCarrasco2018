...
function runEndoTexDetect_detection(myscreen)
global stimulus % initStimulus in main driver function

%% Set parameters for stimuli
% cue
stimulus.cue.rMin = 0.2;
stimulus.cue.rMax = 0.45;
...
task{1}.parameter.targetPresent = stimulus.targetTilt; % [present absent]
task{1}.random = 1;
...
if myscreen.blocks.n==myscreen.blocks.nBlocks
    if strcmp(myscreen.subj,'JP') || strcmp(myscreen.subj,'MJ_cardinalTilts')
...
% get dimensions of cue
[stimulus.cue.x0, stimulus.cue.y0] = pol2cart(targTheta,stimulus.cue.rMin);
[stimulus.cue.x1, stimulus.cue.y1] = pol2cart(targTheta,stimulus.cue.rMax);
...
%% Response cue is just the exogenous cue
...
switch task.thistrial.thisseg
        ...
        if task.thistrial.cue
            ...
            if stimulus.cue.dispNum>0
                mglLines2(stimulus.cue.x0,stimulus.cue.y0,stimulus.cue.x1,...
                    stimulus.cue.y1,stimulus.fixation.size,[0 0 0]);
            end
        else
            % draw frame
    ...
    case 8 % response cue
        % made response cue just like the exogenous cue
...
if find(task.thistrial.buttonState)~=task.thistrial.targetPresent
...
if task.thistrial.targetEcc<3 && ~task.thistrial.cue && task.thistrial.targetPresent==1
    updateStair = 1;
else
    updateStair = 0;
end
...
% create target or noise
switch task.thistrial.targetPresent
    case 1
        % present
        gaborAngle = 90;
        target = 1;
...
temp = make2ndOrderTex(stimulus.carrier.sf,stimulus.gabor.sf,...
    ...
    task.thistrial.targetHemi,target,'gaborAngle',gaborAngle,...
