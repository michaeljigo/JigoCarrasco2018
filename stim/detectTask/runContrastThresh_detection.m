...
function runContrastThresh_detection(myscreen)
...
task{1}.parameter.targetPresent = [1 2]; % [vertical tilted]
task{1}.random = 1;
...
% create the cue
thetaVal = [pi 0];
targTheta = thetaVal(task.thistrial.targetHemi);
...
updateStair = 0;
if task.thistrial.targetEcc<3 && task.thistrial.targetPresent==1
