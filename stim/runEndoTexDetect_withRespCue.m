...
function runEndoTexDetect_withRespCue(myscreen)
global stimulus % initStimulus in main driver function

%% Set parameters for stimuli
% cue
stimulus.endoCue.rMin = 0.35;
stimulus.endoCue.rMax = stimulus.endoCue.rMin*2;
stimulus.endoCue.xNum = 0.02; stimulus.endoCue.yNum = 0.02;
stimulus.endoCue.color = [0 0.5 0.13];
stimulus.endoCue.size = 3;
...
stimulus.locCue.yDistFromTarg = 1.6;
...
% parameters
task{1}.parameter.cue = stimulus.parameter.cue; %0=neutral; 1=endogenous cue
...
stimulus.brokenTrial.blinkBreak = [];
stimulus.brokenTrial.distBreak = [];
...
% determine cue position for location cue
...
if task.thistrial.cue
    % get dimensions of endogenous cue
    [stimulus.endoCue.x0, stimulus.endoCue.y0] = pol2cart([targTheta noiseTheta],...
        stimulus.endoCue.rMin);
    [stimulus.endoCue.x1, stimulus.endoCue.y1] = pol2cart([targTheta noiseTheta],...
        stimulus.endoCue.rMax);
end

% display number at origin
stimulus.endoCue.dispNum = [find(unique(stimulus.gabor.ecc)==task.thistrial.targetEcc) ...
    find(unique(stimulus.gabor.ecc)==noiseEcc)];
stimulus.endoCue.dispNum = stimulus.endoCue.dispNum-1;
...
switch task.thistrial.thisseg
    ...
    case 4 % first cue
        if task.thistrial.cue
            if task.thistrial.targetInterval==1
                % target cue
                x0 = stimulus.endoCue.x0(1);
                x1 = stimulus.endoCue.x1(1);
                y0 = stimulus.endoCue.y0(1);
                y1 = stimulus.endoCue.y1(1);
                dispNum = stimulus.endoCue.dispNum(1);
            else
                % noise cue
                x0 = stimulus.endoCue.x0(2);
                x1 = stimulus.endoCue.x1(2);
                y0 = stimulus.endoCue.y0(2);
                y1 = stimulus.endoCue.y1(2);
                dispNum = stimulus.endoCue.dispNum(2);
            end
            % display number at origin
            mglTextDraw(num2str(dispNum),[stimulus.endoCue.xNum,stimulus.endoCue.yNum]);
            if dispNum>0
                mglLines2(x0,y0,x1,y1,stimulus.endoCue.size,stimulus.endoCue.color);
            end
        else
            % draw neutral cue
            % upper line
            mglLines2(stimulus.frame.x0,stimulus.frame.y,...
                ...
                stimulus.endoCue.size,stimulus.endoCue.color);
    ...
    case 8 % location cue for interval 1
        if task.thistrial.targetInterval==1 % target
            ...
            mglLines2(stimulus.locCue.x0(1),...
                stimulus.locCue.y0(1)+stimulus.locCue.yDistFromTarg,stimulus.locCue.x1(1),...
                stimulus.locCue.y1(1)+stimulus.locCue.yDistFromTarg+stimulus.locCue.length,...
        ...
        elseif task.thistrial.targetInterval==2 % cue random location
            ...
            mglLines2(stimulus.locCue.x0(2),...
                stimulus.locCue.y0(2)+stimulus.locCue.yDistFromTarg,stimulus.locCue.x1(2),...
                stimulus.locCue.y1(2)+stimulus.locCue.yDistFromTarg+stimulus.locCue.length,...
                stimulus.locCue.size,stimulus.locCue.color);
            % draw lower line
            mglLines2(stimulus.locCue.x0(2),...
                stimulus.locCue.y0(2)-stimulus.locCue.yDistFromTarg,stimulus.locCue.x1(2),...
    ...
    case 12 % interval 2
        if task.thistrial.targetInterval==2
            ...
            mglBltTexture(stimulus.noise.texture,[0 0]);
        end
    ...
    case 16
        ...
        mglFillRect(stimulus.fixation.loc(1),stimulus.fixation.loc(2),...
            stimulus.fixation.size-stimulus.fixation.edgeOffset,stimulus.endoCue.color);
        mglFillRect(stimulus.fixation.loc(1),stimulus.fixation.loc(2),...
            fliplr(stimulus.fixation.size)-stimulus.fixation.edgeOffset,stimulus.endoCue.color);
end
...
if stimulus.eye.eyeTrack && ismember(task.thistrial.thisseg,3:length(task.thistrial.seglen)-1)
    ...
    if ~ieNotDefined('dist')
        ...
        if sum(stimulus.eye.avgDistFromFix==inf)>=blinkThresh && all(stimulus.eye.avgDistFromFix(end:-1:end-(blinkThresh-1))==inf)% 1
            ...
            breakTrial = 1;
            stimulus.brokenTrial.blinkBreak(task.trialnum) = 1;
            disp('blink break');
        elseif length(stimulus.eye.avgDistFromFix)>=stimulus.eye.breakTrialTime
            ...
            if avgDist>stimulus.eye.breakTrial
                breakTrial = 1;
                disp('dist break');
                stimulus.brokenTrial.distBreak(task.trialnum) = 1;
            end
        end
...
stimulus.targetEcc(task.trialnum,:) = [task.thistrial.targetEcc; task.thistrial.targetHemi];
...
temp = make2ndOrderTex(stimulus.carrier.sf,stimulus.gabor.sf,...
    ...
    task.thistrial.targetHemi,1,'gaborSD',stimulus.gabor.sd);
...
%     stimulus.noise.all = nan(task.numTrials,size(temp,1),size(temp,2));
% end
% stimulus.target.all(task.trialnum,:,:) = temp;
% stimulus.noise.all(task.trialnum,:,:) = noiseTemp;
