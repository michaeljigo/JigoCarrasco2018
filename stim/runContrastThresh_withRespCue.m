...
function runContrastThresh_withRespCue(myscreen)
...
% exogenous cue (used for location cue here)
stimulus.exoCue.yDistFromTarg = 1.6;%stimulus.gabor.sd+(stimulus.gabor.sd/2); % this ensures that the bar will be 0.3 degrees above one standard deviation of gabor
stimulus.exoCue.size = 3;%0.5;
stimulus.exoCue.color = [0 0.5 0.13];
...
cueISI = stimulus.soa;
stim = 0.1;%0.1; % 10 frames (larger than maximum duration from Yeshurun & Carrasco, 2000)
resp = inf;
respISI = 0.2;
makeTex = inf;
locCue = 0.2;
...
task{1}.numTrials = stimulus.trialnum; %arbritrarily set for now

% parameters
task{1}.parameter.cue = 0; %0=neutral
task{1}.parameter.targetEcc = stimulus.gabor.ecc;
task{1}.parameter.targetHemi = [2 1]; % [right left]
task{1}.parameter.targetInterval = [1 2]; % [first second]
task{1}.random = 1;
...
switch task.thistrial.thisseg
    ...
    case 8 % location cue for inerval 1
...
if stimulus.eye.eyeTrack && ismember(task.thistrial.thisseg,3:length(task.thistrial.seglen)-1)
    ...
    breakTrial = 0;
    blinkThresh = 3; % blinkThresh nans in a row are required for a blink to register
