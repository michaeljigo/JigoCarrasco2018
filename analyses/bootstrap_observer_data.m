% Purpose:  Perform bootstrapping analysis on individual observer data and generate distribution of fitted peak eccentricities.
%
% By:       Michael Jigo
% Edited:   07.01.21
%
...
function bootstrap_observer_data(subj,exp_name)
...
bootDist = cellfun(@(h,f) sort(norminv(h)-norminv(f)),...
    squeeze(temp.bootstrap(2,:,:)),squeeze(temp.bootstrap(1,:,:)),...
    'UniformOutput',false);
...
groupFolder = ['../data/bootstrap/',exp_name,'/'];
if ~exist(groupFolder,'dir')
    mkdir(groupFolder);
end

save([groupFolder,subj,'.mat'],'peakDist');

fprintf('Saved bootstraps for %s.\n',subj);

function peak = getFitPeak(x,y,p)
fitX = linspace(min(x),max(x),1e6);
