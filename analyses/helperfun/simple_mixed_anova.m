function [tbl,rm,gen_eta_squared] = simple_mixed_anova(datamat, varargin)
...
% add generalized eta squared for each effect
%% NOTE: This computation assumes all factors are manipulated between subjects.
F = tbl{3:2:end,4};
df_ef = tbl{3:2:end,2};
df_er = tbl{4:2:end,2};
gen_eta_squared = (F.*df_ef)./(F.*df_ef+df_er);
gen_eta_squared = [tbl.Properties.RowNames(3:2:end) num2cell(gen_eta_squared)];
